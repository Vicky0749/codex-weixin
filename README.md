...
/api key <编号或名称>         仅密钥管理员本人查看指定 API 的完整密钥
...
`/api` 会显示每个已保存 API 的模型、地址和密钥后四位。`/api add` 会等待 2 分钟。期间下一条纯文本只用于验证 API，不会发送给 Codex 或写入会话记录；验证成功后使用 Windows DPAPI 加密保存，默认不自动切换。需要启用时再发送 `/api <编号>` 或 `/api use <名称>`。每个 API 独立保存默认模型和推理强度；切换 API 时会应用这组默认值，并清除旧会话的模型与推理强度覆盖。密钥管理员必须在本机管理台的微信账号页面从已授权联系人中手动指定；只有该联系人可使用 `/api key <编号或名称>` 接收完整密钥。微信不提供可靠的阅后即焚能力，收到密钥后应手动删除该消息。未知指令会提示发送 `/help`。
